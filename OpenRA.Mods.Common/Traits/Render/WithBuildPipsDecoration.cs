...
{
	public class WithBuildPipsDecorationInfo : WithDecorationBaseInfo
	{
		[FieldLoader.Require]
		...
		public override object Create(ActorInitializer init) { return new WithBuildPipsDecoration(init.Self, this); }
	}

	public class WithBuildPipsDecoration : WithDecorationBase<WithBuildPipsDecorationInfo>
	{
		readonly Animation pips;


		public WithBuildPipsDecoration(Actor self, WithBuildPipsDecorationInfo info) : base(self, info)
		{
		...
		protected override IEnumerable<IRenderable> RenderDecoration(Actor self, WorldRenderer wr, int2 screenPos)
		{
			...
			var finishedAmount = 100;

			Foundation foundation = self.Trait<Foundation>();


			screenPos -= pipSize / 2;
			for (var pipIndex = 0; pipIndex < Info.PipCount; pipIndex++)
			{
				var currentProgress = foundation.BuildProgress * Info.PipCount;
				pips.PlayRepeating(currentProgress > pipIndex * finishedAmount ? Info.FullSequence : Info.EmptySequence);
