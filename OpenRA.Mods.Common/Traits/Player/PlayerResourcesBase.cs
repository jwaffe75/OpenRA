...
{
	public abstract class PlayerResourcesBaseInfo : TraitInfo, ILobbyOptions
	{
	...
	public abstract class PlayerResourcesBase : ISync
	{
		readonly Player owner;

		protected PlayerResourcesBase(Actor self, PlayerResourcesBaseInfo info)
		{
			owner = self.Owner;
			InsufficientFundsNotificationInterval = info.InsufficientFundsNotificationInterval;
			InsufficientFundsNotification = info.InsufficientFundsNotification;
			InsufficientFundsTextNotification = info.InsufficientFundsTextNotification;
		...
		public virtual int InsufficientFundsNotificationInterval
		{
			get; protected set;
		}

		public virtual string InsufficientFundsNotification
		{
			get; protected set;
		}

		public virtual string InsufficientFundsTextNotification
		{
			get; protected set;
		}
		...
		public bool TakeCash(int num, bool notifyLowFunds = false)
		{
			...
			{
				if (notifyLowFunds && Game.RunTime > lastNotificationTime + InsufficientFundsNotificationInterval)
				{
					lastNotificationTime = Game.RunTime;
					Game.Sound.PlayNotification(owner.World.Map.Rules, owner, "Speech", InsufficientFundsNotification, owner.Faction.InternalName);
					TextNotificationsManager.AddTransientLine(owner, InsufficientFundsTextNotification);
				}
